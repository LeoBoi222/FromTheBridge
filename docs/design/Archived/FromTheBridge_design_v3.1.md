...
(see `docs/plans/2026-03-09-instrument-admission-design.md`). Instruments enter at
collection tier, promote to signal_eligible only after passing admission criteria
(≥180d derivatives depth, ≥90% completeness, spot price available, ≥10 metrics from
≥3 sources). Coverage grows organically from data quality, not static lists.
...
**`source_id` type convention:** ClickHouse `source_id` (String) stores the
`canonical_name` value, not the PostgreSQL UUID. PostgreSQL SQL examples that filter
by source should use `canonical_name`, not `source_id` directly.
...
CREATE TABLE collection_events (
    ...
    source_id               UUID        NOT NULL REFERENCES source_catalog (source_id),
...
CREATE TABLE instrument_metric_coverage (
    ...
    metric_id           UUID            NOT NULL REFERENCES metric_catalog (metric_id),
