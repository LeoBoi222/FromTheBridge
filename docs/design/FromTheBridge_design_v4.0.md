...
## Empire Architecture v4.0

**Date:** 2026-03-09
**Upgraded from:** v3.1 (2026-03-06)
**Type:** Single Source of Truth — standalone, all layers, all threads, all session decisions
**Owner:** Stephen (architect, sole operator)
**Status:** Authoritative. All locked decisions require architect approval to reopen.

> **SSOT Authority:** This document is the single source of truth for the FromTheBridge
> data platform. No satellite document, session plan, or summary (including CLAUDE.md)
> may claim authority over any design decision. Each fact lives in exactly one place —
> here. Session plans in `docs/Historical/` are evidence trails, not authority. CLAUDE.md
> summarizes this document for agent context but does not override it. Migration files
> implement what this document specifies.
>
> **Going-forward rules:**
...
> 4. Session decisions must be pulled into this document before the session is considered complete
> 5. No satellite document may claim authority over any slice of the design

> This document synthesizes thread_1 through thread_7, thread_infrastructure, and all
> session decisions (solo-ops, instrument admission, T5 patch, T3b backfill readiness,
> cross-source assessment) into one canonical reference. Every decision in every build
> session must trace to a section here. If a build requirement cannot be traced here,
...
Layer 7: Catalog
  ...
  12 relational tables. No time series data here — ever.
...
EDS is a sibling project that runs on-chain node infrastructure and exchange API
collection on shared proxmox hardware. EDS owns the `empire.*` ClickHouse database
and `empire_utxo` PostgreSQL schema. FTB owns `forge.*` in both databases.

**Shared infrastructure cohabitation:**

| Resource | FTB Ownership | EDS Ownership |
|----------|---------------|---------------|
| ClickHouse (`empire_clickhouse`) | `forge` database: observations, dead_letter, current_values | `empire` database: 12 tables (observations, raw_blocks, etc.) |
| PostgreSQL (`empire_postgres`) | `forge` schema: 12 catalog tables | `empire_utxo` schema: UTXO tracking state |
| Dagster | FTB asset groups (collection, export, ops) | EDS asset groups (`eds_*` prefix) |
| MinIO | `bronze-hot`, `bronze-archive`, `gold` buckets | None (v1) |

**Data bridge: `empire_to_forge_sync`**

EDS derives on-chain metrics (MVRV, SOPR, NUPL, Puell Multiple, transfer volume)
from full node data and syncs promoted metrics to `forge.observations` via a Dagster
asset (`empire_to_forge_sync`, 6h cadence). The sync writes with
...
Promoting an EDS metric to `forge.metric_catalog` is a deliberate, manual operation:
1. Developer adds row to `forge.metric_catalog` via SQL migration in FTB repo
2. Developer adds `eds_derived` source to `forge.source_catalog` if not present
3. `empire_to_forge_sync` recognizes the metric on next run and begins flowing data
4. FTB adapter for that metric decommissioned per §Adapter Decommission Protocol

The sync asset does not auto-register metrics. Promotion requires architect approval
plus 7-day EDS freshness record plus dead-letter rate <1%.

**Mutual exclusion for overlapping metrics:**

When EDS replaces an FTB data source for a metric (e.g., node-derived MVRV replaces
BGeometrics MVRV), no dual-write occurs. Decommission sequence: EDS metric validates
7 days → FTB adapter disabled for that metric → `source_catalog` updated.

**Cross-source metric candidates (Phase 2 evaluation):**

Five EDS-derived metrics that require per-exchange granularity are candidates for
promotion to `forge.metric_catalog` after Phase 2 gate:

| Metric | Target Pillar | Rationale |
|--------|---------------|-----------|
| `exchange.aggregate.funding_rate_dispersion` | Structural Risk | Venue stress signal invisible in Coinalyze aggregate |
| `exchange.aggregate.oi_migration` | Liquidity/Flow | Distinguishes migration from deleveraging |
| `exchange.liquidation.concentration` | Structural Risk | Herfindahl index — systemic vs isolated cascade |
| `cross.flow_liquidation_lag` | Structural Risk | Leading indicator of leveraged stress |
| `cross.onchain_vs_exchange_volume_divergence` | Liquidity/Flow | EDS version preferred (node-derived data) |

**Promotion prerequisites (all must be met before any candidate enters metric_catalog):**
1. EDS-59 (cross-exchange aggregation) and EDS-60 (cross-track derivation) live
2. Base `empire_to_forge_sync` operating successfully ≥7 days on existing metrics
3. Candidate metric has ≥7 days of observations in `empire.observations`
4. Dead-letter rate <1% for the candidate metric
5. Architect approval

8 EDS-exclusive metrics are derivable in FTB's Marts layer from data FTB already
collects — these are NOT imported pre-derived. Pipeline item: LH-70.

**EDS cohesion audit resolution summary:**

| Finding | Resolution |
|---------|------------|
| C1 (CH user naming) | FTB uses `ch_writer` consistently |
| C2 (instrument_id nullability) | EDS uses `'__market__'` (non-null); FTB CH uses NULL; sync maps |
| C3 (metric_id naming) | EDS v1.1.2 adopts FTB canonical names for all synced metrics |
| C4 (domain CHECK) | `chain` and `valuation` domains added to FTB CHECK |
| R1 (data_version collision) | Mutual exclusion protocol documented above |
| G8/G9 (Dagster integration) | Tracked by EDS-47/EDS-48, must resolve before Phase 1 |
...
### Rule 2: No application service reads forge.* in ClickHouse

Only Dagster assets read the `forge` database in ClickHouse. No application service
(API, UI, worker) has ClickHouse credentials. Two scoped read credentials exist:
...
- `ch_ops_reader` — Dagster health monitoring assets (`ftb_ops.*`, `shared_ops.*`).
  ...
  `shared_ops` assets are materialized by FTB's Dagster code location:
  `shared_ops.capacity`, `shared_ops.hardware`, `shared_ops.risk_board`,
  `shared_ops.sync_health`, `shared_ops.container_health`,
  `shared_ops.calendar_cleanup`. These monitor cross-project resources
  (proxmox containers, ClickHouse, PostgreSQL) shared between EDS and FTB.

All analytical workloads go through Gold (Iceberg on MinIO, read by DuckDB).

**Scope:** Rule 2 applies to the `forge` database only. The `empire` database
(EDS) has its own access policies governed by EDS design rules. EDS reads
`empire.*` via its API — this does not violate Rule 2.

**Enforcement:** ClickHouse credentials are scoped per-user with SETTINGS PROFILEs
(see §Credential Isolation). No Docker service outside Dagster has a ClickHouse
connection string. ClickHouse query log records all connections — unexpected
client IPs are immediately visible.
...
### Rule 3: No time series data in forge.* PostgreSQL tables

The `forge` schema in PostgreSQL holds relational integrity only. No
`observed_at + value` columns exist in any `forge.*` table. No metric observations,
no derived computations, no feature values.

**Scope:** Rule 3 applies to the `forge` schema. The `empire_utxo` schema on the
same PostgreSQL instance is governed by EDS Rule 4, which inherits the same
principle. UTXO tracking tables store entity lifecycle state (creation_timestamp,
spending_timestamp), not metric observations. Derived time-series values from UTXO
data are written to `empire.observations` in ClickHouse, not PostgreSQL.
...
- Public GitHub repo: metric catalog schema (sanitized), PIT-correctness README, EDSx
  ...
  not open source.
- 16-week GTM sprint: ~27 hours total (~1.7h/week). All GTM work batched during natural
  build pauses.

**16-Week GTM Sprint Timeline:**
...
| 4 | Phase 3 live. Deploy 48h preview infrastructure (activated Phase 4 shadow week 2). | 2h |
...
### Coverage Framing and Instrument Admission Framework
...
completeness thresholds in Phase 1 via the Instrument Admission Framework.

**Core principle:** Admission criteria are canonical. Instrument lists are ephemeral.
Counts that appear in this document are outputs of criteria, not inputs. The criteria
govern, not the list.
...
Maps to `instruments.collection_tier` CHECK constraint. No DDL change required.

**Admission criteria for signal-eligible promotion (all must pass simultaneously):**
...
Demotion is automated. Demoted instruments retain historical data — re-promotion
possible when criteria are met again.

**Stablecoins and wrapped assets:** USDC, USDT are stablecoin supply/peg feeds for the
Liquidity/Flow pillar at `__market__` level. Not instruments. WBTC is a DeFi protocol
metric (TVL, peg ratio), not an independent derivatives instrument. All three are
metric inputs, not scoring candidates.

**Coinalyze instrument discovery:** Adapters backfill all instruments returned by API
discovery, not a fixed list. The "121 instruments" count from v3.1 is retired — the
actual count is determined by adapter discovery and admission criteria evaluation.

**Auto-promote/auto-demote Dagster asset:** Weekly scheduled asset evaluates all
`collection` and `scoring` instruments against admission criteria. Instruments crossing
thresholds are promoted/demoted automatically. All changes logged in `collection_events`
with `event_type = 'promotion_candidate'` or `'demotion'` for audit trail.

**Phase 0 seed (minimal):**
...
No instrument starts as signal_eligible. Promotion happens automatically when
admission criteria are met during Phase 1 collection.

**Phase 1 gate criterion:** ≥12 instruments at signal_eligible tier, all passing
admission criteria. Evidence: 8 evidence-backed (BTC, ETH, SOL, BNB, AVAX, UNI,
LINK, AAVE) + 4 probationary candidates = 12-14 realistic ceiling.
...
| GTM sprint | ~27 hours / 16 weeks. Batched during build pauses. (F1) |
...
**Granularity:** Per-instrument (signal-eligible universe) + market-level aggregate.
...
· `risk_off_crisis` · `transitional` · `recovery` (6 states, empirically determined via BIC/AIC)
...
Layer 2 synthesis algorithm is fully specified in §L2.1–L2.8.
...
#### ML Training Windows (T3b audit findings)

The 450-day forward collection assumption from thread_3 is eliminated. The constraint
is data quality, not data depth. All models have sufficient historical depth.

| Model | Floor Date | Window (years) | Walk-Forward Folds | Status |
|-------|-----------|----------------|-------------------|--------|
...
**Derivatives floor:** Coinalyze pre-Feb-2022 data is degraded (OI at 3.4% of expected
volume). Effective derivatives training floor = Feb 2022, not Mar 2021. Coinalyze
aggregate OI < Binance single-exchange OI from Oct 2025 — investigate before migration.

**Sparse feature policies:**
...
| Source | Target Depth | Est. Volume | Priority |
|--------|-------------|-------------|----------|
| Coinalyze full history | 5yr (2021-03+) | ~283k rows | P1 |
| DeFiLlama TVL | 6yr (2020-05+) | ~40k rows | P1 |
| DeFiLlama DEX volume | 10yr (2016-04+) | ~88k rows | P1 |
| DeFiLlama stablecoins | 9yr (2017-11+) | ~36k rows | P1 |
| DeFiLlama lending (fees + yields) | 5yr (2021+) | ~20k rows | P1 |
| BGeometrics | 13yr BTC / 10yr ETH | ~20k rows | P1 |
| Tiingo OHLCV | 12yr BTC / 10yr ETH | ~50k rows | P0 (unit normalization dep) |
| Binance bulk migration | Already loaded | 96,771 rows → Silver | P1 |
...
Post-graduation operational monitoring. Prevents silent ML decay — the failure mode
that is invisible until a customer notices.

**Dagster asset:** `daily_model_drift_check` — scheduled daily, reads shadow/production
model outputs and feature distributions from Gold.

**Monitored metrics (per model):**

| Metric | Method | Alert threshold | Action |
|--------|--------|-----------------|--------|
| Feature distribution drift | Population Stability Index (PSI) per feature | PSI > 0.25 | Flag for retraining evaluation |
| Prediction calibration error | Expected Calibration Error (ECE) | ECE > 0.05 (graduation threshold) | Automatic flag; manual review required |
| Directional accuracy | Rolling 30-day accuracy on resolved outcomes | Drop > 5pp from graduation baseline | Flag for retraining evaluation |
| Sharpe decay | Rolling 90-day Sharpe of directional calls | Sharpe < 0.3 (below 0.5 graduation floor) | Flag for retraining evaluation |
| Feature importance stability | Top-10 feature overlap vs training snapshot | < 60% overlap | Investigate — possible regime shift or data issue |

**Retraining triggers (any one sufficient):**
- PSI > 0.25 on ≥ 3 features simultaneously
- Directional accuracy drop > 5pp sustained for > 7 days
- ECE > 0.05 sustained for > 7 days
- Manual architect decision

**Retraining is not automatic.** Drift monitor flags; architect reviews; retrained model
enters shadow mode and must re-pass all five graduation criteria before replacing
the production model. Class C methodology change notice (14-day) applies if output
distribution shifts materially.

**Phase gate:** `daily_model_drift_check` must be operational and producing daily reports
by Phase 4 gate. Drift monitor is a Phase 4 gate criterion, not a Phase 1 concern.
...
SignalResponse = {
    ...
    # Operational state (see Signal Operational States)
    "signal_state":        str,             # "healthy"|"degraded"|"impaired"|"suppressed", required
    "frozen_since":        str | None,      # ISO 8601; non-null when signal_state = "impaired" and data_completeness < freeze_threshold
    "frozen_reason":       str | None,      # human-readable; non-null when frozen_since is non-null
    ...
    "provenance": {
        ...
        "feature_contributions":          "list[{feature: str, contribution: float, direction: str}] | None",
        # Ranked feature contributors (SHAP-derived). Stored at signal emission time
        # for explainability endpoint. Top 10 features per instrument. Null until ML graduates.
    },
...
- **`performance_metrics`** — Rolling aggregations per `(instrument_id, track, horizon_days,
  window_days)`. ~60 rows per signal-eligible instrument (scales with universe size). Recomputed daily after `signal_outcomes` update.
...
CREATE TABLE instruments (
    instrument_id       TEXT            PRIMARY KEY,  -- e.g. 'BTC-USD', '__market__'
...
CREATE INDEX idx_instruments_collection_tier ON instruments (collection_tier);
CREATE INDEX idx_instruments_asset_class ON instruments (asset_class);
```

**TEXT PK convention:** `instrument_id` is the human-readable canonical symbol (e.g.
`BTC-USD`, `ETH-USD`, `__market__`). This is the same string written to ClickHouse
`forge.observations.instrument_id`. No join needed to resolve names. The UUID +
`canonical_symbol` design from v3.1 is superseded.
...
A single row in `forge.instruments` with `instrument_id = '__market__'`,
...
series, stablecoin totals, market cap totals.

**ClickHouse convention:** Adapters write `instrument_id = NULL` in
`forge.observations` for market-level metrics (see DDL: `Nullable(String)`).
The `__market__` row exists in PostgreSQL only — for catalog queries, coverage
tracking, and relational integrity. It is never written as a string value to
ClickHouse Silver. EDS uses `'__market__'` as a non-null default in
`empire.observations`; the `empire_to_forge_sync` maps `'__market__'` → `NULL`
when writing to `forge.observations`.
...
CREATE TABLE metric_catalog (
    metric_id           TEXT            PRIMARY KEY,  -- canonical name, e.g. 'derivatives.perpetual.funding_rate'
...
CREATE INDEX idx_metric_catalog_domain ON metric_catalog (domain);
CREATE INDEX idx_metric_catalog_status ON metric_catalog (status);
CREATE INDEX idx_metric_catalog_signal_pillar ON metric_catalog (signal_pillar)
    WHERE signal_pillar IS NOT NULL;
```

**TEXT PK convention:** `metric_id` IS the canonical name (e.g.
`derivatives.perpetual.funding_rate`). The separate `canonical_name` column from
v3.1 is removed — the PK serves that purpose. This is the same string written to
ClickHouse `forge.observations.metric_id`.
...
**EDS metric promotion workflow:**

When EDS derives a metric that FTB consumes, the metric is promoted to the FTB
catalog via manual seed migration. This is not automated by the sync asset.

1. EDS metric meets promotion criteria: architect approval + 7-day freshness
   record + dead-letter rate <1%
2. Developer adds row to `forge.metric_catalog` via SQL migration in FTB repo
   (metric_id, domain, subdomain, source_id = `eds_derived`)
3. Developer verifies `eds_derived` exists in `forge.source_catalog`
4. `empire_to_forge_sync` recognizes the metric on next run and begins flowing
   observations to `forge.observations`
5. FTB adapter for that metric decommissioned per §Adapter Decommission Protocol

**Domain taxonomy note:** The `chain` and `valuation` domains were added to
support EDS-derived on-chain metrics (MVRV, SOPR, NUPL, Puell Multiple, transfer
volume, NVT proxy). The `price` and `metadata` domains support spot pricing and
instrument metadata respectively. See seed migration for the full catalog.

#### metric_lineage

Many-to-many relationship between metrics and their sources, used by Dagster to
construct the asset dependency graph. Each row represents one `(metric_id, source_id)`
pair — a single metric may be collected from multiple sources, and a single source
may provide multiple metrics.
...
CREATE TABLE metric_lineage (
    ...
    compute_agent       TEXT        NOT NULL,       -- 'adapter', 'forge_compute', 'dbt'
    compute_version     TEXT,
    input_metrics       TEXT[],                     -- upstream metric_ids for derived metrics
    formula_ref         TEXT,                       -- methodology doc or code path
    is_primary          BOOLEAN     NOT NULL DEFAULT true,  -- primary source for this metric
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    deprecated_at       TIMESTAMPTZ,
...
CREATE INDEX idx_metric_lineage_source ON metric_lineage (source_id);
CREATE INDEX idx_metric_lineage_compute_agent ON metric_lineage (compute_agent);
```

**Dagster asset construction:** Each `(metric_id, source_id)` row maps to one Dagster
Software-Defined Asset with key `collect_{source_id}_{metric_domain}`. The `input_metrics`
array defines upstream dependencies for derived metrics (e.g., NVT proxy depends on
market_cap and transaction_count). The `is_primary` flag indicates which source is
authoritative when multiple sources provide the same metric.

#### source_catalog

```sql
CREATE TABLE source_catalog (
    source_id               TEXT        PRIMARY KEY,  -- canonical name, e.g. 'coinalyze', 'fred'
...
CREATE INDEX idx_source_catalog_tier ON source_catalog (tier);
CREATE INDEX idx_source_catalog_redistribution ON source_catalog (redistribution_status);
```

**TEXT PK convention:** `source_id` IS the canonical name (e.g. `coinalyze`, `fred`).
The separate `canonical_name` column from v3.1 is removed. This is the same string
written to ClickHouse `forge.observations.source_id`. No UUID-to-string mapping needed.
...
CREATE TABLE collection_events (
    ...
    source_id               TEXT        NOT NULL REFERENCES source_catalog (source_id),
    metric_id               TEXT        REFERENCES metric_catalog (metric_id),
...
CREATE INDEX idx_collection_events_source ON collection_events (source_id);
CREATE INDEX idx_collection_events_status ON collection_events (status);
CREATE INDEX idx_collection_events_started ON collection_events (started_at);
```
...
(
    ...
    value               Nullable(Float64),          -- NULL for categorical/event metrics
...
(~40 GB at 5 years, archive-only). Total estimated 5yr: ~192 GB against 4TB SSD (95% headroom).
...
When EDS provides a node-derived replacement for a metric currently sourced by an
FTB adapter (e.g., EDS node-derived MVRV replaces BGeometrics MVRV), the FTB
adapter for that metric is decommissioned. No dual-write. Adapters may be
**partially decommissioned** — some metrics removed while others remain active
from the same source.

**Decommission sequence:**

1. EDS metric validates for 7 consecutive days (freshness + dead-letter rate <1%)
2. Architect approves promotion (see §Metric promotion in Sibling Project section)
3. `forge.metric_catalog` row added via SQL migration with `source_id = 'eds_derived'`
4. `empire_to_forge_sync` begins flowing data on next run
5. FTB adapter disabled for that specific metric:
   - If adapter serves multiple metrics: remove the metric from the adapter's
     asset definition, keep remaining metrics active
   - If adapter serves only this metric: disable the entire adapter
     (`is_active = false` on source_catalog entry)
6. `forge.source_catalog` updated: original source's `metadata` JSONB field annotated with
   decommission date and replacement source (`eds_derived`)
7. `forge.instrument_metric_coverage` rows for the old source marked
   `is_active = false`
8. Historical data from the original source remains in `forge.observations` —
   not deleted. ReplacingMergeTree deduplication handles the transition naturally
   since source_id differs between old and new data.

**Rollback:** If EDS metric degrades after decommission, re-enable the FTB adapter.
The original adapter code remains in the repository (not deleted) for 90 days
after decommission. After 90 days of stable EDS operation, adapter code may be
archived.

**Example:** BGeometrics currently provides MVRV, SOPR, NUPL, Puell Multiple. When
EDS derives these from Bitcoin UTXO tracking, the `collect_bgeometrics` asset is
disabled for those metrics. If BGeometrics has no remaining active metrics, the
source is marked inactive.

**Candidate metrics for EDS replacement (known):** `chain.valuation.mvrv_ratio`,
`chain.valuation.sopr`, `chain.valuation.nupl`, `chain.valuation.puell_multiple`
(currently BGeometrics), `chain.activity.transfer_volume_usd` (currently
CoinMetrics).
...
**Domain mapping:** Gold domain `onchain` maps to catalog domain `chain`. Gold domain
...
**Volume at full v1 buildout:** ~5,800–6,000 Silver rows/day from FTB adapters (excluding BLC-01 tick aggregation). Per-export delta
(normal): ~1,300–1,600 rows. Annual Silver accumulation: ~2.1M rows (Year 1).

**After EDS sync activation:** `empire_to_forge_sync` adds ~17,350 rows/day
(EDS Track 1/2/3 combined). Total Silver volume rises to ~23,000+ rows/day.
Resource projections in this document account for FTB-only volume; adjust when
sync goes live (EDS-1 gate).
...
(instrument universe determined by adapter discovery and admission criteria)
...
`X-API-Key` header. API key required on ALL endpoints including Free tier (except 48h
preview — unauthenticated by design) — enables uniform audit trail and rate limiting.
No unauthenticated bypass on keyed endpoints.
...
|---|---|---|---|---|---|
| Endpoints | /v1/market/prices, /v1/macro, /v1/instruments, /v1/health | All Free + /v1/signals, /v1/signals/{id}, /v1/signals/performance, /v1/regime | All Signal API + /v1/features/{id}, /v1/signals/{id}/explain | All Signal API + /v1/liquidations (BLC-01) | All Signal API |
...
| Signal fields | N/A | Composite direction + confidence + confidence_tier + signal_state + frozen_since + frozen_reason + pillar breakdown + ML components (per module access) | All Signal API + raw feature values + provenance trace (incl. feature_contributions) | Signal API fields + real-time BLC-01 | Signal API fields scoped to customer instruments |
...
-- Note: Phase 5 migration only needed if Phase 0 was deployed without
-- redistribution columns (pre-v3.1 deployments).
...
`magnitude`, `horizon`, `as_of`, `staleness_flag`, `signal_state`, `frozen_since`,
and `frozen_reason`.
...
#### Signal Explainability Endpoint (Phase 5)

**`GET /v1/signals/{instrument_id}/explain`**
Ranked feature contributors for the current signal. Returns the top features driving
the composite score, with contribution magnitude and direction. Intelligence Suite
tier only.
...
```json
{
    ...
    "feature_contributions": [
        {"feature": "funding_rate_8h_zscore", "contribution": 0.18, "direction": "bullish"},
        {"feature": "exchange_netflow_7d", "contribution": 0.14, "direction": "bearish"},
        {"feature": "hy_oas_change_30d", "contribution": 0.11, "direction": "bullish"}
    ],
    "total_features_used": 47,
    "ml_graduated": true
}
```

**Implementation:** Sorted slice of the `provenance.feature_contributions` field
(stored at signal emission time via SHAP values). No post-hoc computation — reads
pre-materialized data from `marts.signals_history`.

**Data dependency (confirmed at Phase 3 gate):** `feature_contributions` must be
populated in `marts.signals_history` at signal emission time. If ML has not graduated,
returns EDSx pillar-level contributions only (coarser granularity, 5 pillars × weight).

**Tier access:** Intelligence Suite only. Signal API and below receive 403.
...
composite. Horizons: all valid per track (1d/7d/30d for EDSx, 14d for ML). ~60
rows per signal-eligible instrument (scales with universe size).
...
performance on the long tail of the signal-eligible universe.
...
Redis, no Memcached). ~2 KB per signal-eligible instrument (scales linearly). GIL-safe atomic swap on refresh.
...
| `GET /v1/signals/{instrument_id}/explain` | < 20ms | < 50ms | < 100ms |
...
| Sustained RPS | 20 | Steady-state throughput under normal load. Above this, latency degrades. |
| Burst limit | 100 | Short-burst ceiling (≤5s). Requests beyond this queue or 429. |
...
| Cold-start time | < 2s | MinIO snapshot fetch + cache hydration on container start |
| Memory budget (FastAPI worker) | 512 MB | Signal cache ~250 KB + DuckDB in-process + Python overhead |
| Max response size | 5 MB | Full universe signal snapshot + provenance (signal-eligible universe × ~2 KB each, headroom for explainability) |
...
**Preview traffic isolation (design note):** At v1 scale, unauthenticated preview
endpoints (`/v1/market/prices`, `/v1/macro`, `/v1/instruments`, `/v1/health`) serve pre-materialized
data with negligible query load — no isolation needed. Before general availability,
preview traffic should move to a separate worker or CDN cache layer. Trigger: when
concurrent paid API users exceed 10 or preview traffic exceeds 5 RPS sustained.
Pipeline item to track.
...
| **Impaired** | `data_completeness < 0.35` for a single instrument (fewer than 35% of contributing sources fresh), OR signal compute pipeline failed | Freeze last good snapshot for affected instruments | Frozen snapshot served with `is_stale: true` + `signal_state: "impaired"` + `frozen_reason` + `frozen_since` (ISO 8601) | SLA 3 + explicit "impaired" status page update |
...
**Freeze behavior:** `signal_state = "impaired"` is the operational state (inputs are
degraded). `frozen_since` non-null indicates the system response (held the last valid
snapshot rather than recomputing). These are separate concepts — "impaired" describes
what happened to the inputs, "frozen" describes what the system did about it. API
consumers check `signal_state` for the condition and `frozen_since` for the response.
...
| Intelligence Suite-only | `GET /v1/features/{id}`, `GET /v1/signals/{id}/explain`, provenance trace, window=all |
...
| Explainability endpoint | `GET /v1/signals/{id}/explain`. Pre-materialized SHAP contributions. Intelligence Suite only. Phase 5 scope. Data dependency confirmed at Phase 3 gate. |
...
7. Seed structural instruments (BTC, ETH, SOL spot at `collection` tier + `__market__` system instrument). Full instrument universe populated by Phase 1 adapter discovery and admission criteria (see `docs/plans/2026-03-09-instrument-admission-design.md`).
...
| Historical depth | Every active metric in `forge.metric_catalog` has non-NULL `backfill_depth_days`. `SELECT COUNT(*) FROM forge.metric_catalog WHERE status = 'active' AND backfill_depth_days IS NULL` returns 0. |
| Priority-1 backfill | Coinalyze, DeFiLlama, BGeometrics, Tiingo, Binance bulk all have ≥2yr Silver history |
...
| Cross-source evaluation | 5 EDS cross-source metric candidates evaluated for promotion (see §Cross-source metric candidates). Decision recorded for each. |
...
2. Implement pillar scoring for all 5 pillars (trend_structure, liquidity_flow,
   valuation, structural_risk, tactical_macro — implement in priority order per
   REM roadmap)
...
**Phase 3/4 parallel execution (approved):** Phase 3 (EDSx) and Phase 4 (ML) run in
parallel after Phase 2 gate. They share no state — EDSx is deterministic rule-based,
ML is probabilistic model-based. Net savings: ~1-2 weeks. Convergence at Phase 5
(Signal Synthesis and Serving).
...
| Model drift monitor | `daily_model_drift_check` Dagster asset operational; producing daily PSI, ECE, accuracy, Sharpe reports for all 5 models |
...
| Magnitude | Non-null for all signal-eligible instruments. Contingent on ML Phase 4 graduation. If ML not graduated, magnitude requirement waived for Phase 5. |
...
| Explainability endpoint | `GET /v1/signals/{id}/explain` returns ranked feature contributions; Intelligence Suite tier only; data dependency on `provenance.feature_contributions` confirmed |
...
consumer, projected ~75 GB over 5 years (including hot tier rotation). ClickHouse on the 2TB SSD — compressed
...
| FastAPI (Phase 5) | empire_fastapi | 8000 | — |
...
| User | Access | SETTINGS PROFILE | Mounted On |
|---|---|---|---|
| `ch_writer` | INSERT on `forge.observations` + `forge.dead_letter` only. No SELECT. | `ch_writer_profile`: max_memory=4GB, max_threads=4, max_execution_time=300s | `empire_dagster_code` only |
| `ch_export_reader` | SELECT on `forge.observations`, `forge.dead_letter`, `forge.current_values`. No INSERT. | `ch_export_reader_profile`: max_memory=2GB, max_threads=2, max_execution_time=120s | `empire_dagster_code` only |
| `ch_ops_reader` | SELECT on `forge.observations` (metadata/counts only), `forge.dead_letter`, `forge.current_values`. No INSERT. | `ch_ops_reader_profile`: max_memory=1GB, max_threads=1, max_execution_time=30s | `empire_dagster_code` (health assets only) |
| `ch_admin` | All (DDL + admin) | None | Never mounted — operator terminal only |

**Resource profiles** prevent unbounded resource consumption on the shared ClickHouse
instance. EDS defines equivalent profiles for `eds_writer` (4GB/4 threads) and
...
ClickHouse is shared with EDS (`empire` database). Resource profiles prevent either
project from monopolizing the instance. EDS applies matching limits on its users.

| User | `max_memory_usage` | `max_threads` | `max_execution_time` |
|---|---|---|---|
| `ch_writer` | 4 GB | 4 | 300s |
| `ch_export_reader` | 2 GB | 2 | 120s |
| `ch_ops_reader` | 1 GB | 1 | 30s |

Applied via `ALTER USER ... SETTINGS PROFILE` in the credential isolation migration.
EDS users (`eds_writer`, `eds_reader`) self-limit to matching profiles per EDS design.
...
- `ch_export_reader` mounted in `empire_dagster_code` only — no other service has
  ClickHouse SELECT credentials (Rule 2)
- `ch_writer` not mounted on `empire_dagster_code` — export reads Silver, never
  writes back
...
|------------|-----------|---------|----------|
| `POSTGRES_FORGE_USER_PASSWORD` | `secrets/pg_forge_user.txt` | `empire_dagster_code`, `empire_fastapi` | Annual (March) |
| `POSTGRES_FORGE_READER_PASSWORD` | `secrets/pg_forge_reader.txt` | `empire_fastapi`, MCP server | Annual (March) |
| `CH_WRITER_PASSWORD` | `secrets/ch_writer.txt` | `empire_dagster_code` | Annual (March) |
| `CH_EXPORT_READER_PASSWORD` | `secrets/ch_export_reader.txt` | `empire_dagster_code` (export asset only) | Annual (March) |
| `MINIO_BRONZE_WRITER_ACCESS_KEY/SECRET` | `secrets/minio_bronze_key.txt`, `secrets/minio_bronze_secret.txt` | `empire_dagster_code` | Annual (March) |
| `MINIO_ARCHIVE_WRITER_ACCESS_KEY/SECRET` | `secrets/minio_export_key.txt`, `secrets/minio_export_secret.txt` | `empire_dagster_code` | Annual (March) |
| `MINIO_EXPORT_WRITER_ACCESS_KEY/SECRET` | `secrets/minio_export_key.txt`, `secrets/minio_export_secret.txt` | `empire_dagster_code` (export asset) | Annual (March) |
| `MINIO_MARTS_WRITER_ACCESS_KEY/SECRET` | `secrets/minio_marts_key.txt`, `secrets/minio_marts_secret.txt` | `empire_dagster_code` (dbt + forge_compute) | Annual (March) |
| External API keys (Tiingo, etc.) | `secrets/external_apis/{source}.txt` | `empire_dagster_code` | Per-provider policy |
| `CLOUDFLARE_TUNNEL_TOKEN` | `secrets/cf_tunnel_token.txt` | `cloudflared` service | On compromise only |
...
- Current projection: ~72,000 rows/day (including BLC-01 tick-level collection); 1B threshold = ~38 years at current volume
...
### Solo Operator Operations

**Context:** Single operator (Stephen). Every failure mode must be detectable without
human monitoring. The dominant failure mode is silent data degradation — sources
return partial data, adapters succeed but miss instruments, Bronze writes succeed but
Silver writes fail. These cascade through the pipeline silently.
...
| Adapter collects partial data | 2 | Yes | Yes — same cascade, harder to detect |
...
**Dagster ops asset namespaces:**

| Namespace | Owner | Assets |
|-----------|-------|--------|
| `ftb_ops.*` | FTB | Adapter freshness, export pipeline |
| `shared_ops.*` | FTB (materializes all) | Capacity, hardware SMART, risk board, calendar cleanup, alert dispatch, sync boundary, container health |
| `eds_ops.*` | EDS | Node health, EDS adapter health, sync pipeline |

FTB's Dagster instance materializes all `shared_ops.*` assets (prevents duplicate writes).

**Phase 1 health assets:**

`ftb_ops.adapter_health` — one asset per source:

| Field | Source | Alert Condition |
|-------|--------|-----------------|
| `last_observation_at` | MAX(observed_at) from forge.observations | Exceeds cadence_hours × 2 |
| `observations_24h` | Count in last 24h | Zero or <50% expected |
| `dead_letter_24h` | Count from forge.dead_letter | >10 |
| `metric_coverage_pct` | Distinct metric_ids observed / expected | <80% |
| `instrument_coverage_pct` | Distinct instrument_ids / expected | <80% |

`ftb_ops.export_health` — Silver-to-Gold export:

| Field | Source | Alert Condition |
|-------|--------|-----------------|
| `last_export_at` | Dagster run history | >2h ago |
...
**FTB red alert triggers:**
- Silver-to-Gold export failed 3 consecutive times
- ClickHouse merge lag >600s
- >3 adapters simultaneously returning zero data
- MinIO unreachable
- Dead letter rate >5%

**Phase 1 runbooks (FTB-01 through FTB-08):**
...
Full runbooks stored in `docs/runbooks/`. Each includes: severity, detection, impact,
immediate action, resolution steps (exact commands), verification, post-mortem.

**Ops credentials (3 new roles, created during Phase 1 infra):**

| Role | Scope | Exception |
|------|-------|-----------|
| `calendar_writer` | INSERT + SELECT on `forge.event_calendar` only | EDS writes `forge.event_calendar` via this role — sole exception to "EDS never writes forge.*" |
| `risk_writer` | INSERT + SELECT on `empire.risk_assessment` only | FTB writes `empire.risk_assessment` via this role |
| `ch_ops_reader` | SELECT on `forge.observations`, `forge.dead_letter`, `system.merges`, `system.parts` | See §Credential Isolation |

**event_calendar schema extension (deployed Phase 1):**
...
-- Expanded event_type CHECK (14 values):
...
**Future phase gate ops requirements (not specified in advance — written when phase is built):**
- Phase 2: dbt model failure runbooks, Gold compaction, feature null investigation
- Phase 4: ML retraining and feature drift runbooks
- Phase 5: serving and tunnel runbooks, end-to-end red alert within 5 minutes
...
|--------|----------|------------|----------------|
| Coinalyze | Perpetual futures — funding, OI, liquidations, L/S ratio (universe per adapter discovery) | Unaudited | Pending Phase 6 audit |
...
**Internal derived source:**
| EDS (eds_derived) | Node-derived on-chain metrics via `empire_to_forge_sync` | N/A (internal) | Yes |
